...
class DeleteRecord extends Component
{
    ...
    public function isOkPermission($action)
    {
        if (!Yii::$app->request->isPost) {
            ...
            $bitacora->registerAndFlash($event, 'isOkPermission', MSG_SECURITY_ISSUE);
     ...
     * @return bool
     */
    public function isOkSeleccionItems($result)
    {
        if (!isset($result)) {
            $event = Yii::t(
                'app',
            ...
            $bitacora = new Bitacora();
            $bitacora->registerAndFlash($event, 'isOkSeleccionItems', MSG_SECURITY_ISSUE);
            return false;
        }
        return true;
    }
    ...
    {
        $okTransaction = 0; // 0: OK was deleted
        if ($model == null) {
        ...
        if ($okTransaction == 0) {
            try {
                ...
                $okTransaction = 1;
                $bitacora = new Bitacora();
                $bitacora->register($exception, 'app\composer\DeleteRecord::remove', MSG_ERROR);
            }
        }
    ...
    public function summaryDisplay($status)
    {
        ...
        if (isset($ids{2})) {
            ...
            $this->summaryDisplayItem($msg, $ids, MSG_SUCCESS);
        }
        $ids = $status[1];
        if (isset($ids{2})) {
            $msg = 'Selected records: \'{ids}\' a problem occurred removing the record';
            $this->summaryDisplayItem($msg, $ids, MSG_ERROR);
        }
        $ids = $status[2];
        if (isset($ids{2})) {
    ...
    public function summaryDisplayItem($msg, $ids, $statusId)
    {
